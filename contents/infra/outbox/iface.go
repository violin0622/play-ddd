...
// EventRepo is interface between outbox processor and event out persisitent storage.
// (e.g. Postgres or MySQL)
// It is assumed that events are arranged in an orderly manner by time overall,
// and individual aggregation events are strictly sorted in the order they occur.
...
// The amount of Events and Result in each call are not guaranteed to be the same,
// but the order must be the same.
type EventsBatch interface {
	// Poll events limited by arg, in order. Events don't have to be in same aggregation.
